...
```{r}
load("~/rproj/Paper3Annot_2024/ClinVar20240107/20240227_refseq_snpeff/GRCh37_BLB/df_ClinVar_GRCh37_BLB_snpeff_refseq.RData")
load("~/rproj/Paper3Annot_2024/ClinVar20240107/20240227_refseq_snpeff/GRCh37_PLP/df_ClinVar_GRCh37_PLP_snpeff_refseq.RData")
```
...
```{r}
bind_rows(
  df_ClinVar_GRCh37_BLB_snpeff_refseq,
...
```{r}
CSQ_ClinVar_GRCh37_BLB_snpeff_refseq <- df_ClinVar_GRCh37_BLB_snpeff_refseq %>%
  select(key, NM, Annotation) %>%
  separate_rows(Annotation, sep = "&") %>%
  distinct() %>%
  group_by(key, NM) %>%
...
```{r}
save(CSQ_ClinVar_GRCh37_BLB_snpeff_refseq, file = "CSQ_concordance/CSQ_ClinVar_GRCh37_BLB_snpeff_refseq.RData")
```


```{r}
CSQ_ClinVar_GRCh37_PLP_snpeff_refseq <- df_ClinVar_GRCh37_PLP_snpeff_refseq %>%
  select(key, NM, Annotation) %>%
  separate_rows(Annotation, sep = "&") %>%
  distinct() %>%
  group_by(key, NM) %>%
...
```{r}
save(CSQ_ClinVar_GRCh37_PLP_snpeff_refseq, file = "CSQ_concordance/CSQ_ClinVar_GRCh37_PLP_snpeff_refseq.RData")
```
