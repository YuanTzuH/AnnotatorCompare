...
```{r}
CSQ_ClinVar_GRCh38_BLB_snpeff_ensembl <- df_ClinVar_GRCh38_BLB_snpeff_ensembl %>%
  select(key, ENST, Annotation) %>%
  separate_rows(Annotation, sep = "&") %>%
  distinct() %>%
  group_by(key, ENST) %>%
...
```{r}
save(CSQ_ClinVar_GRCh38_BLB_snpeff_ensembl, file = "CSQ_concordance/CSQ_ClinVar_GRCh38_BLB_snpeff_ensembl.RData")
```


```{r}
CSQ_ClinVar_GRCh38_PLP_snpeff_ensembl <- df_ClinVar_GRCh38_PLP_snpeff_ensembl %>%
  select(key, ENST, Annotation) %>%
  separate_rows(Annotation, sep = "&") %>%
  distinct() %>%
  group_by(key, ENST) %>%
...
```{r}
save(CSQ_ClinVar_GRCh38_PLP_snpeff_ensembl, file = "CSQ_concordance/CSQ_ClinVar_GRCh38_PLP_snpeff_ensembl.RData")
```
