...
```{r}
load("~/rproj/Paper3Annot_2024/ClinVar20240107/20240226_ensembl_vep/GRCh37_BLB/df_ClinVar_GRCh37_BLB_vep_ensembl.RData")
load("~/rproj/Paper3Annot_2024/ClinVar20240107/20240226_ensembl_vep/GRCh37_PLP/df_ClinVar_GRCh37_PLP_vep_ensembl.RData")
```
...
```{r}
bind_rows(
  df_ClinVar_GRCh37_BLB_vep_ensembl,
  df_ClinVar_GRCh37_PLP_vep_ensembl
) %>%
  select(Consequence) %>%
...
bind_rows(
  ...
  select(key, Feature, Consequence, HGVSc, HGVSp) %>%
...
bind_rows(
  ...
  filter(str_length(Consequence) < 50)
```
有HGVSp
太難搞，Others
...
```{r}
CSQ_ClinVar_GRCh37_BLB_vep_ensembl <- df_ClinVar_GRCh37_BLB_vep_ensembl %>%
  mutate(
  ...
  mutate(Feature = str_extract(Feature, '[^\\.]+'))
```

```{r}
save(CSQ_ClinVar_GRCh37_BLB_vep_ensembl, file = "CSQ_concordance/CSQ_ClinVar_GRCh37_BLB_vep_ensembl.RData")
```


```{r}
CSQ_ClinVar_GRCh37_PLP_vep_ensembl <- df_ClinVar_GRCh37_PLP_vep_ensembl %>%
  mutate(
    Consequence = case_when(
...
```{r}
save(CSQ_ClinVar_GRCh37_PLP_vep_ensembl, file = "CSQ_concordance/CSQ_ClinVar_GRCh37_PLP_vep_ensembl.RData")
```
