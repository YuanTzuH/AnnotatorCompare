...
```{bash}
head -n 40 GRCh37_BLB/ClinVar_GRCh37_BLB_Tx1_20240107.vep.vcf
```
...
```{r}
tsv_query <- read_tsv(
  file("GRCh37_BLB/ClinVar_GRCh37_BLB_Tx1_20240107.query.tsv"), 
  col_names = c(
    "chr",
    "start",
    "ref",
...
```{r}
df_ClinVar_GRCh37_BLB_vep_refseq <- tsv_query %>%
  select(
...
# 只留NM, NR, NC Tx
```{r}
df_ClinVar_GRCh37_BLB_vep_refseq <- df_ClinVar_GRCh37_BLB_vep_refseq %>%
...
```{r}
save(df_ClinVar_GRCh37_BLB_vep_refseq, file = "GRCh37_BLB/df_ClinVar_GRCh37_BLB_vep_refseq.RData")
```
