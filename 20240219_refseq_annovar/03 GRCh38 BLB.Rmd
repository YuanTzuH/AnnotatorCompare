...
```{bash}
head GRCh38_BLB/ClinVar_GRCh38_BLB.refGene.variant_function
```
...
```{r}
length(unique(tsv_all_variant$key))

# 120198 Same as Input
```
...
```{bash}
head GRCh38_BLB/ClinVar_GRCh38_BLB.refGene.exonic_variant_function
```
...
```{r}
df_ClinVar_GRCh38_BLB_annovar_refseq <- bind_rows(
...
```{r}
df_ClinVar_GRCh38_BLB_annovar_refseq[df_ClinVar_GRCh38_BLB_annovar_refseq == ""] = "."
df_ClinVar_GRCh38_BLB_annovar_refseq[is.na(df_ClinVar_GRCh38_BLB_annovar_refseq)] = "."
```
...
```{r}
save(df_ClinVar_GRCh38_BLB_annovar_refseq, file = "GRCh38_BLB/df_ClinVar_GRCh38_BLB_annovar_refseq.RData")
```
